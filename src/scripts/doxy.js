...
/**
    @brief Generator of doxygen doc.
    @param data The text content to parse.
*/
...
//! @brief Regex to guess the name of the concerned variable / function.
//! @todo adapt to every langage?
Doxygen.prototype.languages = {
    javascript: [
            /var ([a-zA-Z0-9_]*)/,
            /([a-zA-Z0-9_\.]*)(\s)*=/,
...
Doxygen.prototype.guessKey = function guessKey( line )
{
    ...
    for( var i = 0, size = this.languages[LANGUAGE].length; i < size; ++i )
    {
        var regex = this.languages[LANGUAGE][i];
...
Doxygen.prototype.Keywords = {
    'param' : function( infos, k, d ) {
        ...
        infos['param'].push([ paramName, remainder ]);
    },

    'see' : function( infos, k, d ) {
        infos['see'] = d.split(' ');
    },

    'constructor': function(infos, k, d) {
        infos['Is a constructor'] = 'yes';
    },
...
Doxygen.prototype.parse = function parseComment( comment )
{
    ...
    for( var i = 0, size = comment.length; i < size; ++i )
    {
        ...
        if( line.length > 0 && line[0] === '@' )
        {
            ...
            if (firstSpace != -1) {
                kw = line.substr( 1, firstSpace-1 );
                rest = line.substring( firstSpace+1 );
            } else {
                kw = line.substr(1, line.length);
                rest = '';
            }
    ...
    while( pos != total_length )
    {
        ...
        // We're looking for a "/ * *" mark or a "/ / !" or a "/ / /" mark
        {
            ...
            if( !comment_mode )
            {
                ...
                var objectId = infos.id || infos.var || this.guessKey( nextLine );
...
Doxygen.prototype.htmlreplacers = {
    'brief' : 'Brief',
    'var': 'Variable',
    'id': 'Identifier',
    'return': 'Returns',
...
Doxygen.prototype.toHtml = function webPresent( ) {
    ...
    for( var i in comments )
    {
        ...
        html += '<h3 id="'+i+'">' + i + "</a></h3>\n";
        html += '<p class="backtotop"><a href="#top">top</a></p>';
        html += '<ul>';
        for( var k in comment )
        {
            if( k === 'param' ) {
                ...
                html += '<strong>See also</strong>: ';
                for( var i in comment[k] ) {
                    var withLink = links[comment[k][i]] || false;
                    if( withLink ) {
                        html += '<a href="#' + comment[k][i] + '">';
                    }
                    html += comment[k][i];
                    if( withLink ) {
                        html += '</a>';
                ...
                html += "<li><strong>";
                var replacement = this.htmlreplacers[k] || k;
                html += replacement + "</strong> : " + comment[k] + "</li>\n";
            }
        }
        html += '</ul>\n';
    }
